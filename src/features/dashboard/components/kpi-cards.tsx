...
export function KpiCards({ kpis, insights }: KpiCardsProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-4">
      <KpiCard
...
function KpiCard({
  ...
  return (
    <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-card to-muted/10 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group rounded-xl">
      <div className="flex flex-col h-full p-4 sm:p-5 md:p-6 space-y-3 sm:space-y-4 md:space-y-5">
        ...
        <div className="flex items-end justify-between gap-2">
            <div className="flex flex-col min-w-0">
                <div className="text-2xl sm:text-3xl md:text-3xl lg:text-4xl font-semibold tracking-tight text-foreground tabular-nums transition-all duration-300">
                    {value}
                </div>
            </div>
            
            {/* Micro-Trend Sparkline */}
            <div className="h-[40px] w-[70px] sm:w-[75px] md:w-[80px] pb-1 opacity-80 group-hover:opacity-100 transition-opacity shrink-0">
